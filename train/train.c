...
/*   Updated: 2024/09/27 18:32:47 by gicomlan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_has_pipe(char **command)
{
	return (*command && (strcmp(*command, "|") == 0));
}

int ft_parse_command(char **command, int *is_pipe)
{
	int index;

	index = 0;
	while (command[index] && \
		(strcmp(command[index], "|") != 0) && \
		(strcmp(command[index], ";") != 0))
		index++;
	*is_pipe = ft_has_pipe(&command[index]);
	if (command[index])
		command[index] = NULL;
	return (index);
}
...
{
	//t_micro_shell  make a struct struct
	int input_fd;
	int exit_code;
	int is_pipe;
	int cmd_len;
