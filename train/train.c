...
/*   Updated: 2024/09/27 21:35:36 by gicomlan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_execute_child_process(t_micro_shell *shell)
{
	...
	if (shell->command.type == TYPE_PIPE)//merci with other if
	{
...
{
	//int status;
	//waitpid(shell->pid, &status, 0); display with hook on wait in struct ?
...
static void	ft_skip_semicolons(t_micro_shell *shell)
{
	...
	{
		shell->argv[shell->index] = NULL; //here
		shell->index++;
	}
}
